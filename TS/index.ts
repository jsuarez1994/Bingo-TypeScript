...
function start():void{
    $('#init').hide('slow');
    $('#stop').show('slow');
    var velocidad:number = parseInt(sessionStorage.getItem('velocidad'));   
...
function cargarDinero(){
    var bote:number = parseInt(sessionStorage.getItem('bote'));
    var l:number = parseInt(sessionStorage.getItem('linea'));
    var linea:number = bote*(l/100);
...
function mostrarNumero():void{
    var numAle:number=Math.floor(Math.random()*numerosBombo.length);
...
function cambiarFormato(num:number){
...
function cargarBoletos(){
    let numB:any = sessionStorage.getItem("numeroUsuarios");
...
function generarBoletos(num:number){
...
function cambiarBoleto(e:any){
    let div = document.getElementById(e.target.getAttribute('name'));
